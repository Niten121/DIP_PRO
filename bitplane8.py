...
four_bit_img = (np.array([int(i[4]) for i in lst],dtype = np.uint8) * 12).reshape(img.shape[0],img.shape[1])
...
finalr2 = cv2.hconcat([four_bit_img,three_bit_img,two_bit_img,one_bit_img])
final = cv2.vconcat([finalr,finalr2])
cv2.imshow('bit plane complete',final)
cv2.waitKey(0)